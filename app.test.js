...
describe('renderSidebar', () => {
    ...
    test('filters prompts based on text in title or content', () => {
        ...
        const categoryDivs = Array.from(sidebarContent.children).filter(div => div.style.display !== 'none');
        expect(categoryDivs.length).toBe(1);
        expect(categoryDivs[0].querySelector('h3').textContent).toBe('Category 2');

        const promptItems = Array.from(sidebarContent.querySelectorAll('li')).filter(li => li.style.display !== 'none');
        expect(promptItems.length).toBe(1);
        expect(promptItems[0].querySelector('button').textContent).toBe('Another Prompt');

        // Test filtering by title
        app.renderSidebar(mockCategories, 'prompt 1');
        const promptItems2 = Array.from(sidebarContent.querySelectorAll('li')).filter(li => li.style.display !== 'none');
        expect(promptItems2.length).toBe(1);
        expect(promptItems2[0].querySelector('button').textContent).toBe('Prompt 1');
    });
