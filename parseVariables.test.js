...
describe('parseVariables function', () => {
    test('should extract a basic variable without hint', () => {
        ...
        const replaceRegex0 = new RegExp(`\\[${result[0].raw.replace(/[-/\\^$*+?.()|[\]{}]/g, '\\$&')}\\]`, 'g');
        expect("This is a [TOPIC] prompt.".replace(replaceRegex0, 'fun')).toBe("This is a fun prompt.");
    });
    ...
    test('should extract multiple basic variables', () => {
        ...
        const replaceRegex0 = new RegExp(`\\[${result[0].raw.replace(/[-/\\^$*+?.()|[\]{}]/g, '\\$&')}\\]`, 'g');
        const replaceRegex1 = new RegExp(`\\[${result[1].raw.replace(/[-/\\^$*+?.()|[\]{}]/g, '\\$&')}\\]`, 'g');
        expect("A [TOPIC] for [AUDIENCE].".replace(replaceRegex0, 'fun').replace(replaceRegex1, 'devs')).toBe("A fun for devs.");
    });
    ...
    test('should deduplicate identical variables', () => {
        ...
        const replaceRegex0 = new RegExp(`\\[${result[0].raw.replace(/[-/\\^$*+?.()|[\]{}]/g, '\\$&')}\\]`, 'g');
        expect("A [TOPIC] prompt about the same [TOPIC].".replace(replaceRegex0, 'JS')).toBe("A JS prompt about the same JS.");
    });
    ...
    test('should extract variable name and hint separated by colon', () => {
        ...
        const replaceRegex0 = new RegExp(`\\[${result[0].raw.replace(/[-/\\^$*+?.()|[\]{}]/g, '\\$&')}\\]`, 'g');
        expect("Write about [TOPIC: Data Science].".replace(replaceRegex0, 'AI')).toBe("Write about AI.");
    });
    ...
    test('should handle empty brackets correctly', () => {
        ...
        const replaceRegex0 = new RegExp(`\\[${result[0].raw.replace(/[-/\\^$*+?.()|[\]{}]/g, '\\$&')}\\]`, 'g');
        expect("This is an empty [] bracket.".replace(replaceRegex0, 'test')).toBe("This is an empty test bracket.");
    });
    ...
    test('should handle complex mixed variables', () => {
        ...
        const replaceRegex0 = new RegExp(`\\[${result[0].raw.replace(/[-/\\^$*+?.()|[\]{}]/g, '\\$&')}\\]`, 'g');
        const replaceRegex1 = new RegExp(`\\[${result[1].raw.replace(/[-/\\^$*+?.()|[\]{}]/g, '\\$&')}\\]`, 'g');
        const replaceRegex2 = new RegExp(`\\[${result[2].raw.replace(/[-/\\^$*+?.()|[\]{}]/g, '\\$&')}\\]`, 'g');
        const replaceRegex3 = new RegExp(`\\[${result[3].raw.replace(/[-/\\^$*+?.()|[\]{}]/g, '\\$&')}\\]`, 'g');
        const replacedContent = content
            .replace(replaceRegex0, 'Draft')
            .replace(replaceRegex1, 'book')
            .replace(replaceRegex2, 'science')
            .replace(replaceRegex3, 'students');
    ...
    test('should correctly escape regex special characters in variables', () => {
        ...
        const replaceRegex0 = new RegExp(`\\[${result[0].raw.replace(/[-/\\^$*+?.()|[\]{}]/g, '\\$&')}\\]`, 'g');
        const replaceRegex1 = new RegExp(`\\[${result[1].raw.replace(/[-/\\^$*+?.()|[\]{}]/g, '\\$&')}\\]`, 'g');
        expect(content.replace(replaceRegex0, '10').replace(replaceRegex1, '20')).toBe("Calculate 10 using 20.");
    });
    ...
    test('should handle variables with leading/trailing spaces', () => {
        ...
        const replaceRegex0 = new RegExp(`\\[${result[0].raw.replace(/[-/\\^$*+?.()|[\]{}]/g, '\\$&')}\\]`, 'g');
        expect(content.replace(replaceRegex0, 'Alice')).toBe("Hello Alice.");
    });
    ...
    test('should handle hints containing separator characters', () => {
        ...
        const replaceRegex0 = new RegExp(`\\[${result[0].raw.replace(/[-/\\^$*+?.()|[\]{}]/g, '\\$&')}\\]`, 'g');
        expect(content.replace(replaceRegex0, 'example.com')).toBe("Visit example.com.");
    });
});
